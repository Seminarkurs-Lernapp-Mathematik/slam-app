...
class LocalDataSource {
  ...
  Future<Result<void, AppError>> cacheUser(
  ...
  Future<Result<void, AppError>> cacheLernplan(
      String userId, Map<String, dynamic> data) async {
    try {
      ...
      return Failure(
          CacheError('Failed to cache lernplan: $e', stackTrace: st));
    }
  }
  ...
  Future<Result<void, AppError>> cacheSettings(
      String userId, Map<String, dynamic> data) async {
    try {
      ...
      return Failure(
          CacheError('Failed to cache settings: $e', stackTrace: st));
    }
  }
  ...
  Future<Result<List<Map<String, dynamic>>, AppError>> getQuestions(
  ...
  Future<Result<void, AppError>> cacheQuestions(
      String sessionId, List<Map<String, dynamic>> questions) async {
    try {
      ...
      return Failure(
          CacheError('Failed to cache questions: $e', stackTrace: st));
    }
  }
  ...
  Future<Result<void, AppError>> queueOperation(SyncOperation operation) async {
    try {
      final key =
          '${operation.type}_${operation.id}_${DateTime.now().millisecondsSinceEpoch}';
      await _syncQueue?.put(key, operation.toJson());
      return const Success(null);
    } catch (e, st) {
      return Failure(
          CacheError('Failed to queue operation: $e', stackTrace: st));
    }
  }
  ...
  Future<Result<List<SyncOperation>, AppError>> getPendingOperations() async {
    try {
      ...
      return Failure(
          CacheError('Failed to get pending operations: $e', stackTrace: st));
    }
  }
  ...
  Future<Result<void, AppError>> removeOperation(String key) async {
    try {
      ...
      return Failure(
          CacheError('Failed to remove operation: $e', stackTrace: st));
    }
  }
  ...
  Future<Result<void, AppError>> clearSyncQueue() async {
    try {
      ...
      return Failure(
          CacheError('Failed to clear sync queue: $e', stackTrace: st));
    }
  }
...
class SyncOperation {
  ...
  factory SyncOperation.fromJson(Map<String, dynamic> json) => SyncOperation(
        id: json['id'],
        type:
            SyncOperationType.values.firstWhere((e) => e.name == json['type']),
