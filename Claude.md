# SLAM Learning App - Claude Code Guide
...
1. **Be Specific About Location**
   - Good: "Update the API key handling in live_feed_screen.dart line 67"
   - Bad: "Fix the API key thing"

2. **Reference Existing Patterns**
   - "Use the same Riverpod pattern as in settings_providers.dart"
   - "Follow the error handling in ai_service.dart"

3. **Provide Context**
   - "The feed isn't working because the backend URL is wrong"
   - "We need to support both Claude and Gemini API keys"

4. **Use Clear Commands**
   - "Run dart analyze and fix all warnings"
   - "Add a new route for the whiteboard feature"
   - "Update the theme to use Material 3 colors"

### Common Requests

**Feature Implementation:**
```
"Add a streak freeze purchase button to the shop screen.
It should:
- Cost 50 coins
- Allow users to buy up to 3 streak freezes
- Update Firestore with the purchase
- Show a success animation
- Follow the existing shop pattern"
```

**Bug Fixes:**
```
"The Live Feed crashes when generating questions. Debug by:
1. Checking if API keys are configured
2. Verifying the backend URL is correct
3. Adding error logging to the generateQuestions call
4. Testing with a demo question as fallback"
```

**Refactoring:**
```
"Refactor the question_session_screen.dart to:
- Extract the hint button logic into a separate widget
- Use a state machine for question states
- Add unit tests for the answer evaluation logic"
```
...
```
"Before making changes, run flutter analyze to see current issues"
```

2. **Test After Changes**
```
"After updating the API integration, test with both Claude and Gemini keys"
```

3. **Document Major Changes**
```
"Update the Claude.md file to document the new authentication flow"
```

4. **Use Git for Safety**
```bash
git checkout -b feature/new-canvas
# Make changes
git add .
git commit -m "Add collaborative canvas feature"
```
...
### Project-Specific
- `docs/feature-liste.md` - Complete feature documentation
- `docs/BACKEND_ANALYSIS.md` - API reference and backend structure
- `TEST_PROTOCOL.md` - Testing guidelines and protocols
...
- Store in debug panel (Firestore user settings)
- For production: Use environment variables or Firebase Remote Config

### Firebase Rules
- Ensure Firestore security rules are properly configured
- Users should only access their own data
- Validate all writes on the server side

### Authentication
- Email verification is optional (controlled by Debug Panel)
...
3. **Lazy load** large lists with ListView.builder
4. **Cache network images** (already configured)
5. **Profile with DevTools** for performance issues
...
1. **Share context:** "I'm working on the Live Feed feature in lib/features/live_feed/"
2. **Be specific:** Include file paths, line numbers, error messages
3. **Provide logs:** Copy relevant terminal output or stack traces
4. **Reference docs:** "According to the feature-liste.md, the feed should..."

### Internal Resources

- **Feature Documentation:** `docs/feature-liste.md`
- **Backend API Reference:** `docs/BACKEND_ANALYSIS.md`
- **Test Protocol:** `TEST_PROTOCOL.md`
- **Git History:** Check commit messages for context on changes

### External Resources

- Flutter Discord: [discord.gg/flutter](https://discord.gg/flutter)
- Stack Overflow: Tag your questions with `flutter`, `riverpod`, `firebase`
- GitHub Issues: Check the repo for known issues
...
1. Clone the repo and checkout `flutter-app` branch
2. Run `flutter pub get` and `dart run build_runner build`
3. Configure API keys in Debug Panel after first run
4. Read `docs/feature-liste.md` for feature overview
5. Check `TEST_PROTOCOL.md` for testing guidelines

### Before Making Changes

1. Pull latest from `flutter-app` branch
2. Run `flutter analyze` to see current state
3. Create a feature branch: `git checkout -b feature/my-feature`
4. Make changes and test thoroughly
5. Run analysis again before committing

### Communication

- Document major changes in commit messages
- Update relevant docs (this file, feature-liste.md, etc.)
- Add TODO comments for incomplete features
- Use Git issues for tracking bugs and features

---

**Happy Coding! 🚀**

*For questions about this guide, ask Claude Code to explain any section in detail.*
